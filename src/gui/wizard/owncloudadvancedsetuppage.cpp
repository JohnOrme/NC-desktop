...
{
    const auto backgroundColor = palette().window().color();
    const auto syncArrowIconFileName = Theme::instance()->isBranded() ? Theme::hidpiFileName("sync-arrows.png", backgroundColor)
                                                                      : Theme::hidpiFileName(":/client/theme/colored/sync-arrows.png");
    _ui.syncLogoLabel->setPixmap(syncArrowIconFileName);
}
